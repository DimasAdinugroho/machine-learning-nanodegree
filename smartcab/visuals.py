...
def plot_trials(csv):
    ...
    # additional logs
    count_train = list(training_data['trial'])[-1]
    success_train = sum(list(training_data['success']))

    count_test = list(testing_data['trial'])[-1]
    success_test = sum(list(testing_data['success']))
    ...
    if len(testing_data) > 0:
        ...
        logger.info(' the training ended in: {} trials with {} trials count as success'.format(count_train, success_train))
        logger.info(' the testing ended in: {} trials with {} trials count as success'.format(count_test, success_test))
    else:
